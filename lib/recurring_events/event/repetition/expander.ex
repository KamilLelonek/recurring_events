defmodule RecurringEvents.Event.Repetition.Expander do
  ...
  def stream(%{frequency: frequency, interval: interval, start_date: start_date, end_date: end_date, exclusions: exclusions} = repetition) do
    frequency
    |> Repetition.Frequency.stream_of_dates(interval, start_date, end_date, exclusions)
    |> Enum.map(&build_occurence(&1, repetition))
  end

  defp build_occurence(date, repetition) do
